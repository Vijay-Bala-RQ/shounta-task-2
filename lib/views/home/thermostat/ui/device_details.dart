...
class _DeviceDetailsPageState extends State<DeviceDetailsPage> {
        ...
        if (state is ThermostatError) {
          if (mounted) {
            ToastHelper.showToast(
                context: context,
                message: 'Error loading thermostat details. Please try again.',
                isSuccess: false);
          }
        }
      });
    ...
    return CommonBackgroundPage(
      body: BlocBuilder<ThermostatBloc, ThermostatState>(
        builder: (BuildContext context, ThermostatState state) {
          ...
          return Skeletonizer(
            enabled: state is FetchOneThermostatLoading,
            child: Column(
              children: <Widget>[
                Padding(
                  padding:
  ...
  void _navigateToPage(String pageName) {
    ToastHelper.showToast(
        context: context, message: 'Navigating to $pageName page');
  }
}
